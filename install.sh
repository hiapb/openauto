  ...
  restart_openclaw_service() {
      ...
      log_error "服务重载失败，请执行: journalctl -u openclaw -n 80 --no-pager"
      return 1
  }
  ...
  check_core_status() {
      service_is_active openclaw && printf "${GREEN}在线${NC}" || printf "${RED}离线${NC}"
  }

  check_memory_status() {
      container_is_running_exact qdrant && printf "${GREEN}活跃${NC}" || printf "${RED}休眠${NC}"
  }
  ...
  ensure_node20() {
      ...
      log_warn "探针检测到 Node.js 碎片或版本断层，启动清理流程..."
      apt-get remove -y nodejs npm libnode-dev >/dev/null 2>&1 || true
      apt-get autoremove -y >/dev/null 2>&1 || true

      curl -fsSL https://deb.nodesource.com/setup_20.x | bash - >/dev/null 2>&1
      log_info "注入 Node.js 20 与原生编译工具链..."
      apt-get install -y nodejs build-essential python3 >/dev/null 2>&1

      require_cmd node
      require_cmd npm

      if ! node -v | grep -q '^v20\.'; then
          log_error "Node.js 安装完成，但版本不是 v20.x，当前: $(node -v)"
          return 1
      fi
  }

  _get_machine_key() {
      [[ -f /etc/machine-id ]] && cat /etc/machine-id || hostname
  }
  ...
  pause_matrix_node() {
      ...
      record="$(awk -F'|' -v tgt="${alias}" '$1 == tgt {print; exit}' "${MATRIX_DB}")"
  ...
  delete_matrix_node() {
      ...
      awk -F'|' -v tgt="${alias}" '$1 == tgt {found=1} END {exit(found ? 0 : 1)}' "${MATRIX_DB}" || {
          log_error "节点不存在"
          return 1
      }
  ...
  manage_channels() {
      ...
      while true; do
          list_matrix
          echo -e "${CYAN}=================================================${NC}"
          echo -e " ${GREEN}1.${NC} ➕ 部署新节点"
          echo -e " ${YELLOW}2.${NC} ⏸️  暂停节点"
          echo -e " ${GREEN}3.${NC} ▶️  唤醒节点"
          echo -e " ${RED}4.${NC} 彻底销毁节点"
  ...
  install_core() {
      ...
      if apt-cache search --names-only '^libasound2t64$' | grep -q 'libasound2t64'; then
          ...
          log_warn "检测到 t64 环境，已自动切换音频依赖。"
      fi

      apt-get install -y \
          curl wget git xvfb fluxbox x11vnc jq openssl psmisc \
          libnss3 libnspr4 libatk1.0-0 libatk-bridge2.0-0 libcups2 libdrm2 \
          libxkbcommon0 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 \
          libgbm1 "${asound_pkg}" fonts-wqy-zenhei fonts-wqy-microhei >/dev/null 2>&1
      ...
      if ! npm install -g @openclaw/cli > "${npm_log}" 2>&1; then
          log_error "CLI 安装失败，NPM 日志如下："
          echo -e "${YELLOW}==================== NPM ERROR ====================${NC}"
          tail -n 30 "${npm_log}" || true
          echo -e "${YELLOW}===================================================${NC}"
          return 1
      fi
  ...
  SERVICE
      ...
      if service_is_active openclaw; then
          ...
          log_error "核心启动失败，请执行: journalctl -u openclaw -n 80 --no-pager"
          return 1
      fi
  }
